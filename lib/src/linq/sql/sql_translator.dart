...
/// ## Dialect awareness
///
/// The translator delegates the dialect-
/// specific bits to a [SqlDialect] instance.
/// The dialect is responsible for the
/// placeholder format, the substring-search
/// function (INSTR vs STRPOS), the JSON-object
/// construction function (json_object vs
/// jsonb_build_object), etc.
///
/// In d_rocket 2.0.0, the default dialect
/// emits `?` placeholders and SQLite's
/// function names; the Postgres engine
/// passes a `PostgresDialect` that emits
/// `$1, $2, ...` placeholders and Postgres
/// function names. The translator code is
/// shared across engines; only the dialect
/// differs.
library;
...
class SqlTranslator implements ExprVisitor<SqlFragment> {
  ...
  SqlTranslator({
    this.tableAlias = 'u',
    this.dialect = const DefaultDialect(),
  });
  ...
  SqlFragment visitMapLiteral(MapLiteralExpr e) {
    // .f: emits a JSON-construction function
    // call. The function name comes from the
    // dialect: SQLite 3.38+ uses
    // `json_object(...)`; Postgres uses
    // `jsonb_build_object(...)`; MySQL uses
    // `JSON_OBJECT(...)`. The user can then
    // extract fields with `json_extract(map, '$.k')`
    // (SQLite) or `map->'k'` (Postgres).
    //
    // Why JSON: a true SQL "map" type doesn't
    // exist. JSON is the standard surrogate for
    // key-value pairs in modern SQL. The output of
    // this Expr is a TEXT/JSONB string.
    //
    // Engine-not-supported fallback: if the
    // engine is too old to support the JSON
    // function, the SQL will fail at execute
    // time with a clear "no such function" error
    // from the engine. We document this rather
    // than throw at translation time (which would
    // block the rest of the query).
    final String fn = dialect.jsonObjectFunction();
    if (e.entries.isEmpty) {
      return SqlFragment('$fn()', <Object?>[]);
    }
    ...
    return SqlFragment('$fn(${parts.join(', ')})', binds);
  }
